# VASMA Dataset Installation Guide

This guide provides instructions for installing the 11 diverse benchmark datasets used in VASMA (Variational Augmentation with Semantic Manifold Anchoring). We suggest putting all datasets under the same folder (say `$DATA`) to ease management and following the instructions below to organize datasets to avoid modifying the source code. The file structure looks like
...
### VASMA-Specific Datasets

For VASMA's dual-domain augmentation strategy, you will also need to prepare DALL-E generated images for each dataset. The DALL-E images should be organized as follows:

```
$DATA/
|–– imagenet/
|–– dalle_imagenet/        # DALL-E generated images for ImageNet
|–– caltech-101/
|–– dalle_caltech-101/     # DALL-E generated images for Caltech-101
|–– oxford_pets/
|–– dalle_oxford_pets/     # DALL-E generated images for Oxford-Pets
|–– ...                    # Similar structure for all datasets
```

**Note**: The DALL-E generated images are used by VASMA's pixel-space synthesis component for semantic coverage augmentation. If you don't have access to DALL-E generated images, VASMA can still operate using only the Conditional VAE component for feature-space generation.
